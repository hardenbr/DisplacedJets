...
for key in stacks.keys(): 
    for samp in stacks[key]:
        ...
        while iev < thisTree.GetEntries():
            ...
            if thisTree.eventCaloHT < 350: continue
...
#            print list(thisTree.eventNShortTags)
        ...
        hist_scale =  float(options.lumi) * 1000 * float(samp.xsec) /  float(n_analyzed) #* float(n_pass)
#        samp.hist.Sumw2()
...
for key in stacks.keys():
    stack_sum =  rt.TH1F("stack_%s" % str(key), key, 16, 0, 16)
    stack_sum.GetXaxis().SetTitle(options.xlabel)
    stack_sum.GetYaxis().SetTitle(options.ylabel)

    stack_sum.Fill("1 No Vertex",0)
    stack_sum.Fill("2 No Vertex",0)
    stack_sum.Fill("3 No Vertex",0)
    stack_sum.Fill("4+ No Vertex",0)
    stack_sum.Fill("1 Short",0)
    stack_sum.Fill("2 Short",0)
    stack_sum.Fill("3 Short",0)
    stack_sum.Fill("4+ Short",0)
    stack_sum.Fill("1 Medium",0)
    stack_sum.Fill("2 Medium",0)
    stack_sum.Fill("3 Medium",0)
    stack_sum.Fill("4+ Medium",0)
    stack_sum.Fill("1 Long",0)
    stack_sum.Fill("2 Long",0)
    stack_sum.Fill("3 Long",0)
    stack_sum.Fill("4+ Long",0)
    eval("stack_sum.SetLineColor(rt.%s)" % stack_colors[key])
    stack_sum.SetLineWidth(2)
    stack_sum.SetMarkerStyle(20)
    eval("stack_sum.SetMarkerColor(rt.%s)"% stack_colors[key])



    #stack_sum_total = rt.TH1F("stack_total_%s" % str(key), "label", 4, 0, 4)
    for samp in stacks[key]:
        stack_sum.Add(samp.hist)
        print samp.file_name, 'integral:', samp.hist.Integral()
        #stack_sum_total.Add(samp.hist_total)
    if stack_sum.GetMaximum() > max:
        max = stack_sum.GetMaximum()
        if draw_first != None:            
            draw_rest.append(draw_first)
        draw_first = stack_sum
    else:
        draw_rest.append(stack_sum)

print "length of draw_rest", len(draw_rest)


if options.log:
    draw_first.GetYaxis().SetRangeUser(.1, 1e5)
    canvas.SetLogy()
else:
    draw_first.GetYaxis().SetRangeUser(0, 25)

draw_first.Draw("pEHIST")

for ii in draw_rest: ii.Draw("pEHISTsame")
...
max = -1
for key in stacks.keys():
    #    stack_sum =  rt.TH1F("stack_%s" % str(key), "label", 16, 0, 16)
    stack_sum_total = rt.TH1F("stack_total_%s" % str(key), key , 4, 0, 4)
    stack_sum_total.GetXaxis().SetTitle(options.xlabel)
    stack_sum_total.GetYaxis().SetTitle(options.ylabel)
    stack_sum_total.Fill("1 Total",0)
    stack_sum_total.Fill("2 Total",0)
    stack_sum_total.Fill("3 Total",0)
    stack_sum_total.Fill("4+ Total",0)
    eval("stack_sum_total.SetLineColor(rt.%s)" % stack_colors[key])
    stack_sum_total.SetLineWidth(2)
    stack_sum_total.SetMarkerStyle(20)
    eval("stack_sum_total.SetMarkerColor(rt.%s)"% stack_colors[key])

    for samp in stacks[key]:
        #stack_sum.Add(samp.hist)
        stack_sum_total.Add(samp.hist_total)

    if stack_sum_total.GetMaximum() > max:
        max = stack_sum_total.GetMaximum()
        if draw_first != None:
            draw_rest.append(draw_first)
        draw_first = stack_sum_total
    else:
        draw_rest.append(stack_sum_total)

if options.log:
    draw_first.GetYaxis().SetRangeUser(.1, 1e5)
    canvas_tot.SetLogy()
else:
    draw_first.GetYaxis().SetRangeUser(0, 50)
draw_first.Draw("pEHIST")
